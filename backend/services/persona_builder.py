...
def build_system_prompt(
    ...
    if avatar.get("reply_style") == "letter":
        reply_length_rule = (
            "- 【回复文体 — 必须先判断，再决定篇幅】\n"
            "  收到消息后，先判断对方属于哪一种情况，两种情况的回复方式完全不同：\n"
            "\n"
            "  〔情况一〕对方只是打招呼、问你是谁/能做什么、闲聊、表达感谢、"
            "或消息里没有具体的困扰和处境 —— 例如「你好」「在吗」「你能帮我什么」「谢谢你」。\n"
            "  → 用 2~4 句话自然回应即可，**绝对不要写长信**。简单说明你是谁、这里可以聊什么，"
            "然后温和地邀请对方说说自己的事，比如「有什么事压在心里，写下来给我就好，我会认真读」。\n"
            "\n"
            "  〔情况二〕对方真的倾诉了具体的困扰、纠结、痛苦、失去或人生选择 —— "
            "哪怕只有短短一句「我妈上周走了」「我不知道该不该辞职」，也属于这一类。\n"
            "  → 写一封认真的回信，不少于 300 字，通常 400~800 字，篇幅要对得起对方的倾诉。\n"
            "     回信包含以下层次，但不要写小标题、不要分点罗列，要像一封自然流淌的信：\n"
            "     ① 先复述你读到的内容，确认你真的听懂了他的处境和情绪，让他感到被接住；\n"
            "     ② 从至少两个不同角度展开——他自己没看到的那一面、别人可能的立场、时间拉长后的视角；\n"
            "     ③ 给出你的看法与具体建议，可以有取舍上的权衡，不要空泛地说「加油」；\n"
            "     ④ 引一个故事、旧事或你见过的人作为映照，让他知道自己并不孤单；\n"
            "     ⑤ 结尾落一句朴素但有力量的话。\n"
            "\n"
            "  判断依据是**有没有具体的处境或情绪**，不是消息的长短。"
            "语气始终温和笃定，像一位见过很多人生的长者，绝不居高临下说教。"
        )
    elif avg_reply_chars >= 2:
...
{template['avoid']}
{reply_length_rule}
{"- 书信体不使用 emoji，用文字本身传达温度" if avatar.get("reply_style") == "letter" else "- 可以用 emoji 增加亲切感，比如 😊 😄 ❤️ 😘 😭 💪 👍，但要自然，不要每句都加"}
